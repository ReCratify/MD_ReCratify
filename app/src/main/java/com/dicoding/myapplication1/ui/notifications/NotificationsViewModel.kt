        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (errorBody != null && errorBody.startsWith("<html>")) {
                    _allpost.value = PostResponse(status = false, message = "Received HTML response instead of JSON")
                } else {
                    val errorResponse = Gson().fromJson(errorBody, ErrorResponse::class.java)
                    val errorMessage = errorResponse.message
                    _allpost.value = PostResponse(status = false, message = errorMessage)
                }
            } catch (e: Exception) {
                _allpost.value = PostResponse(status = false, message = "An error occurred: ${e.message}")
            } finally {
